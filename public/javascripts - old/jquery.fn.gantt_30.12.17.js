            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                       // if(entry.berth.length>1){
							entries.push('<div class="row name '+entry.berth+' row' + i + (entry.berth ? '' : ' fn-wide') + '" id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '">');
						//alert(entry.berth.length);
						
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + (entry.berth || '') + '</span>');
                       ...
                       /*  if (entry.desc) {
						...
						/* if (entry.fromdate) {
                            entries.push('<div class="row fromdate row' + i + ' " id="RowdId_' + i + '" data-id="' + entry.id + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + entry.fromdate + '</span>');
                            entries.push('</div>');
                        }
						if (entry.todate) {
                            entries.push('<div class="row todate row' + i + ' " id="RowdId_' + i + '" data-id="' + entry.id + '">');
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + entry.todate + '</span>');
                            entries.push('</div>');
                        } */
